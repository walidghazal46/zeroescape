  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-foreground text-xl font-bold">الإحصائيات</h1>
      </div>

      <div className="grid grid-cols-2 gap-3 mb-5">
        <div className="bg-gradient-to-br from-blue-500/10 to-cyan-500/10 border border-blue-500/20 rounded-xl p-4">
          <Clock className="w-5 h-5 text-blue-400 mb-2" />
          <div className="text-2xl font-bold text-foreground tabular-nums">{totalHours.toFixed(1)}</div>
          <div className="text-muted-foreground text-xs mt-0.5">ساعة هذا الأسبوع</div>
        </div>
        ...
        >
          ...
          <div className="text-2xl font-bold text-foreground tabular-nums">{complianceRate}%</div>
          <div className="text-muted-foreground text-xs mt-0.5">نسبة الالتزام</div>
        </div>

        <div className="bg-gradient-to-br from-orange-500/10 to-red-500/10 border border-orange-500/20 rounded-xl p-4">
          <Flame className="w-5 h-5 text-orange-400 mb-2" />
          <div className="text-2xl font-bold text-foreground tabular-nums">{streak}</div>
          <div className="text-muted-foreground text-xs mt-0.5">يوم متتالي</div>
        </div>
        ...
        <div className="bg-gradient-to-br from-violet-500/10 to-purple-500/10 border border-violet-500/20 rounded-xl p-4">
          ...
          <div className="text-2xl font-bold text-foreground tabular-nums">{trend !== null ? `${trend > 0 ? '+' : ''}${trend}%` : '—'}</div>
          <div className="text-muted-foreground text-xs mt-0.5">عن الأسبوع الماضي</div>
        </div>
      </div>

      <div className="bg-card border border-border rounded-xl p-4 mb-5">
        <h3 className="text-foreground text-sm font-medium mb-4">ساعات التركيز الأسبوعية</h3>
          ...
          <BarChart data={weekData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
            ...
            <Bar dataKey="hours" radius={[6, 6, 0, 0]}>
              {weekData.map((entry, index) => (
                <Cell key={`cell-${index}`} fill={index === weekData.length - 1 ? '#15b8a6' : '#cbd5e1'} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      ...
      <div className="bg-card border border-border rounded-xl p-4 mb-5">
        <h3 className="text-foreground text-sm font-medium mb-3">سلوك كسر الجلسات</h3>
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-muted/70 rounded-lg p-3">
            <div className="text-muted-foreground text-xs mb-1">إجمالي محاولات الكسر</div>
            <div className="text-foreground text-xl font-bold tabular-nums">{weeklyBreaks}</div>
          </div>
          <div className="bg-muted/70 rounded-lg p-3">
            <div className="text-muted-foreground text-xs mb-1">إجمالي المحظورات</div>
            <div className="text-foreground text-xl font-bold tabular-nums">{totalBlocked}</div>
          </div>
          <div className="bg-muted/70 rounded-lg p-3 col-span-2">
            <div className="text-muted-foreground text-xs mb-1">أكثر تطبيق حاولت فتحه</div>
            <div className="text-foreground text-sm font-medium truncate">{mostAttemptedApp || 'لا توجد بيانات بعد'}</div>
          </div>
        </div>
      </div>

      <div className="bg-card border border-border rounded-xl p-4 mb-5">
        <h3 className="text-foreground text-sm font-medium mb-3">نقاط الضعف</h3>
        <div className="space-y-2 text-sm">
          <div className="flex items-center justify-between bg-muted/70 rounded-lg p-3">
            <div className="flex items-center gap-2 text-foreground">
              <AlertTriangle className="w-4 h-4 text-amber-400" />
              <span>الساعة الأضعف</span>
            </div>
            <div className="text-foreground font-medium">{weakestHour !== null ? formatHour(weakestHour) : 'لا توجد بيانات'}</div>
          </div>
          <div className="flex items-center justify-between bg-muted/70 rounded-lg p-3">
            <div className="flex items-center gap-2 text-foreground">
            ...
            <div className="text-foreground font-medium">{goalConfig?.label || 'غير محدد'}</div>
          </div>
          {peakHourData && peakHourData.count > 0 && (
            <div className="flex items-center justify-between bg-muted/70 rounded-lg p-3">
              <div className="text-foreground">ذروة المحاولات</div>
              <div className="text-foreground font-medium">{formatHour(peakHourData.hour)} ({peakHourData.count})</div>
            </div>
          )}
        </div>
      </div>
      ...
      {topApps.length > 0 && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          <h3 className="text-foreground text-sm font-medium mb-3">أكثر التطبيقات المحظورة</h3>
          <div className="space-y-2">
            {topApps.map((app) => (
              <div key={app.appId} className="flex items-center justify-between bg-muted/70 rounded-lg p-3">
                <div className="text-foreground truncate max-w-[70%]">{app.appName}</div>
                <div className="text-foreground font-medium tabular-nums">{app.count}</div>
              </div>
            ))}
          </div>
        </div>
